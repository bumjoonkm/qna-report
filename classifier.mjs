...
export const MONITOR_SYS_PROMPT = [
  ...
  '또한 학습 질문이 본문의 대부분(주된 부분)을 차지하면, 앞·뒤에 감정 표현·사담·잡담이 조금 섞여도 "정상질문"이다. 학습무관은 본문 전체 또는 대부분이 학습과 무관할 때 붙인다. 단, 아래 1의 [A] 항목은 정상 질문이 섞여 있어도 무조건 학습무관이다(hard override).',
  '',
  '판정 순서:',
  '',
  '1) "학습무관" (거절 대상):',
  '  [A] 다음은 본문에 정상적인 학습 질문이 섞여 있어도 무조건 학습무관이다(hard override):',
  '   - 외부 콘텐츠 요청: 노래·곡·OST·뮤비 틀어/불러달라, 드라마·소설·웹툰·괴담 줄거리나 등장 소재 소개, 성대모사, 영상편지.',
  '   - TA에게 대가·뇌물 제시: "질문권 선물할게요/드릴게요", "답해주면 ~해줄게요" 등 질문 내용과 무관하게 답변의 대가를 제안하는 멘트(내용이 학습 질문이어도 학습무관).',
  '   - 시대인재 내부 강사(선생님)끼리 비교하거나, 콘텐츠·교재·강의·n제끼리 우열·비교를 묻는 질문(예: "A쌤이랑 B쌤 중 누가 더 나아요?", "이 교재랑 저 교재 중 뭐가 좋아요?"). ※ 단순 추천 요청("추천해주세요")은 여기 아님 → 학습상담(2).',
  '  [B] 다음은 본문의 대부분(주된 부분)이 여기에 해당할 때만 학습무관이다. 정상적인 학습 질문이 본문의 대부분을 차지하면 앞·뒤에 조금 섞여 있어도 정상질문이다:',
  ...
  '   ※ "말투·감정"은 판단 근거가 아니다. 학습 내용을 물으면서 불평/짜증/푸념("질문권 아까워요"), AI 호출("부엉아"), 위로 요청("빌떨된 부엉이 위로해주세요"), 가벼운 비속어("개소리")가 섞여도, 앞·중심에 학습 질문이 있으면 정상질문이다. 예: "N축 오랜만이라 까먹었는데 저기 동그라미 친 부분이 왜 미분 불가인가요? 월례 3점 틀려서 속상한 부엉이 위로해주세요" → 정상질문(앞이 학습 질문).',
  '',
  '2) "학습상담" (거절 안 함) — 특정 문제·개념 자체가 아니라, 공부를 "어떻게/어디서" 할지에 대한 상담·안내:',
  '   - 공부법·학습 방향·계획·성적·멘탈·슬럼프·번아웃·동기·의욕·태도·불안 상담, 컨텐츠/n제/강사 추천. (⚠️ 강사끼리·콘텐츠끼리 "비교·우열"을 묻는 것은 학습상담 아님 → 학습무관, 위 1의 [A].)',
