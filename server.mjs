...
async function fetchMeetSettleMonth(token, year, month, online) {
  ...
  try {
    ...
    let total = 0, count = 0;
    for (const r of rows) {
      if (r.deleteYn === 'Y' || r.cancelYn === 'Y') continue;
      // 기본 쿼리(onlineYn 생략)가 대면+온라인 전체를 반환하는 경우 대비 — row에 onlineYn이 있으면 행 단위 분리
      if (r.onlineYn != null) {
        if (online && r.onlineYn !== 'Y') continue;
        if (!online && r.onlineYn === 'Y') continue;
      }
      if (r.scheduleDt && !String(r.scheduleDt).startsWith(ym)) continue;
      total += Number(r.taMeetConsultationSettleAccountAmount) || 0;
      count++;
    }
...
// 실제 급여 지급 내역 (경영지원 정산 확정표 기준, 2026-08 전달분) — 단위: 원.
// 정산 API 계산치와 별도의 "정답지". 비교 가능한 3개 항목(온라인질문/TA Meet/TA Meet On)만 담고
// SA 멘토링·프로모션은 제외. 페이지 HTML에 하드코딩하지 않고 토큰이 필요한 API 응답에만 실어
// 로그인한 사용자에게만 노출한다.
const ACTUAL_SALARY = {
  2025: {
    3:  { online: 27598900, meet: 62873626, meetOn: 7854176 },
    4:  { online: 58740900, meet: 74057134, meetOn: 18442657 },
    5:  { online: 71695600, meet: 62801049, meetOn: 12611045 },
    6:  { online: 62315100, meet: 47293997, meetOn: 7605271 },
    7:  { online: 99019200, meet: 79361227, meetOn: 7203887 },
    8:  { online: 86460900, meet: 74954515, meetOn: 11660953 },
    9:  { online: 83227400, meet: 44956800, meetOn: 11181607 },
    10: { online: 84144200, meet: 43540544, meetOn: 10083395 },
    11: { online: 37278400, meet: 17775902, meetOn: 4623417 },
  },
};

// 급여 총 비교: refYear(올해) vs prevYear(작년), 3월 ~ effEnd월(직전 완료월로 클램프) 월 단위.
// 온라인 지출은 AI 현황 탭과 동일 경로(fetchSettleMonth + perf2 일별 캐시)를 재사용해 두 탭 숫자가 항상 일치.
async function fetchSalaryCompare(token, endMonth) {
  const kt = kstTodayDt(); // Render(UTC)에서도 KST 기준으로 "직전 완료월" 판정
  const refYear = parseInt(kt.slice(0, 4));
  const curMonth = parseInt(kt.slice(5, 7));
  const prevYear = refYear - 1;
  const effEnd = Math.min(endMonth, curMonth - 1);
  const months = [];
  for (let m = 3; m <= effEnd; m++) months.push(m);
  if (months.length === 0) return { refYear, prevYear, endRequested: endMonth, endEffective: effEnd, months: [] };

  // AI OWL 비용: refYear 해당 월들의 일별 항목에서 aiCostTotal(USD) 합산 (perf2-* 캐시 재사용)
  const allDates = [];
  for (const m of months) {
    const mm = String(m).padStart(2, '0');
    const last = lastDayOfMonth(refYear, m);
    for (let d = 1; d <= last; d++) allDates.push(`${refYear}-${mm}-${String(d).padStart(2, '0')}`);
  }
  const dayMap = {};
  ...
  for (const dt of allDates) {
    ...
    for (const item of (dayMap[dt] || [])) {
      if (AI_TA_IDS.has(item.taId)) aiUsdByMonth[m] = (aiUsdByMonth[m] || 0) + (item.aiCostTotal || 0);
    }
  }

  // 월별 정산 5종: 온라인(25/26) + Meet 대면(25/26) + Meet On(26)
  const jobs = [];
  for (const m of months) {
  ...
  const jobResults = await parallelMap(jobs, (j) => j.run(), 6);
  const byMonth = {};
  jobs.forEach((j, i) => { (byMonth[j.m] = byMonth[j.m] || {})[j.kind] = jobResults[i]; });

  const sumOnlineMonth = (byDate, year, m) => {
    const prefix = `${year}-${String(m).padStart(2, '0')}-`;
    let t = 0;
    for (const [k, v] of Object.entries(byDate || {})) if (k.startsWith(prefix)) t += v;
    return t;
  };
  ...
  const out = months.map((m) => {
    ...
    const act = (ACTUAL_SALARY[prevYear] || {})[m];
    const prevActual = act ? { ...act, total: act.online + act.meet + act.meetOn } : null;
    const actualSavingsPct = prevActual && prevActual.total > 0 ? ((prevActual.total - ref.total) / prevActual.total) * 100 : null;
    return { month: m, prev, ref, savingsPct, meetErrMsg, prevActual, actualSavingsPct };
  });
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/api/salary-compare') {
    const token = url.searchParams.get('token');
...
<div class="app" id="appView">
  <div class="tabs">
    ...
    <div class="tab" onclick="switchTab('salarycmp')">급여 총 비교</div>
  ...
  <!-- 급여 총 비교 -->
  <div class="page" id="page-salarycmp">
    <div class="ctrl">
...
let salaryCmpChartInstance = null;
let salaryCmpActualChartInstance = null;

const salaryTotalsPlugin = {
  id: 'salaryTotals',
  afterDatasetsDraw(chart) {
    const { ctx, data } = chart;
    const months = chart.config._salMonths || [];
    ctx.save();
    ctx.textAlign = 'center';
    data.labels.forEach((_, i) => {
      const stacks = {};
      ...
      ctx.font = 'bold 11px sans-serif';
      ctx.fillStyle = '#333';
      if (stacks.gPrev) ctx.fillText('₩' + stacks.gPrev.total.toLocaleString(), stacks.gPrev.x, stacks.gPrev.y - 8);
...
function initSalaryCmpSelect() {
  const sel = document.getElementById('salcmpEnd');
  if (!sel || sel.options.length > 0) return;
  for (let m = 3; m <= 12; m++) {
    sel.insertAdjacentHTML('beforeend', '<option value="' + m + '">' + m + '월</option>');
  }
  // 기본값 = 직전 완료월 (getMonth()는 0-based라 현재월-1과 동일)
  sel.value = Math.min(Math.max(new Date().getMonth(), 3), 12);
}

function salcmpManwon(v) {
  return Math.round(v / 10000).toLocaleString() + '만';
}

async function doSalaryCompare() {
  if (!TOKEN) return;
  const end = parseInt(document.getElementById('salcmpEnd').value);
  const btn = document.getElementById('salcmpBtn');
  const note = document.getElementById('salcmpNote');
  btn.disabled = true;
  btn.textContent = '조회 중... (최초 조회는 시간이 걸릴 수 있어요)';
  note.style.display = 'none';
  try {
    const res = await fetch('/api/salary-compare?token=' + encodeURIComponent(TOKEN) + '&end=' + end);
    if (!res.ok) throw new Error(await res.text());
    const data = await res.json();
    if (!data.months || data.months.length === 0) {
      note.style.display = 'block';
      note.textContent = '아직 정산이 확정된 달이 없습니다. 3월이 끝난 뒤부터 표시됩니다.';
    } else {
      const msgs = [];
      ...
      renderSalaryCmpActualChart(data);
    }
  } catch (err) { alert('오류: ' + err.message); }
  btn.disabled = false;
  btn.textContent = '조회';
...
// 공용 드로어: months는 {month, prev:{online,meet,meetOn,total,meetError}, ref, savingsPct} 배열.
// 계산치 차트와 실제 내역 차트가 동일한 스택/라벨/플러그인 구성을 공유한다.
function drawSalaryCmpChart(canvasId, prevInstance, months, prevName, refName) {
  ...
  const datasets = [
    { label: prevName + ' 온라인 답변', backgroundColor: '#F1BF42', stack: 'gPrev', data: months.map(mo => mo.prev.online) },
    { label: prevName + ' TA Meet', backgroundColor: '#C8912E', stack: 'gPrev', data: months.map(mo => mo.prev.meet) },
    { label: prevName + ' TA Meet On', backgroundColor: '#8F6A1E', stack: 'gPrev', data: months.map(mo => mo.prev.meetOn) },
    { label: refName + ' 온라인 답변 (AI 포함)', backgroundColor: '#F0908C', stack: 'gRef', data: months.map(mo => mo.ref.online) },
    { label: refName + ' TA Meet', backgroundColor: '#A94442', stack: 'gRef', data: months.map(mo => mo.ref.meet) },
    { label: refName + ' TA Meet On', backgroundColor: '#D9534F', stack: 'gRef', data: months.map(mo => mo.ref.meetOn) },
  ...
  const chart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            title: (items) => months[items[0].dataIndex].month + '월 (' + prevName + ' vs ' + refName + ')',
            label: (item) => {
      ...
      scales: {
        x: { stacked: true, grid: { display: false }, categoryPercentage: 0.7, barPercentage: 0.8 },
        y: { stacked: true, beginAtZero: true, max: yMax, ticks: { callback: (v) => (v / 10000).toLocaleString() + '만' }, title: { display: true, text: '급여 지출' } },
      },
    },
    plugins: [ChartDataLabels, salaryTotalsPlugin],
  });
  chart.config._salMonths = months;
  chart.update();
  return chart;
}
...
// 실제 급여 내역(정답지) vs 올해 계산치 — prevActual이 있는 달만 표시.
function renderSalaryCmpActualChart(data) {
  const card = document.getElementById('salcmpActualCard');
  if (!card) return;
  const months = (data.months || []).filter(mo => mo.prevActual).map(mo => ({
    month: mo.month,
    prev: Object.assign({ meetError: false }, mo.prevActual),
    ref: mo.ref,
    savingsPct: mo.actualSavingsPct,
  }));
  if (months.length === 0) {
    card.style.display = 'none';
    if (salaryCmpActualChartInstance) { salaryCmpActualChartInstance.destroy(); salaryCmpActualChartInstance = null; }
    return;
  }
  card.style.display = '';
  const prevSuffix = String(data.prevYear).slice(2);
